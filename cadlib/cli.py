...
def cmd_build(args: argparse.Namespace) -> None:
    sources = find_parts() if args.target == "all" else find_parts(Path(args.target))
    if not sources:
...
def cmd_show(args: argparse.Namespace) -> None:
    ...
    (source,) = find_parts(Path(args.target))
...
def main() -> None:
    ...
    build.add_argument("target", nargs="?", default="all", help="part file, directory, or 'all'")
    ...
    show.add_argument("target", help="part file")
